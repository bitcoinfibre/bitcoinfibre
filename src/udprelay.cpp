...
static void SendFECedData(const uint256& blockhash, UDPMessageType type, const std::vector<unsigned char>& data, DataFECer& fec) {
    ...
    if (type == MSG_TYPE_BLOCK_HEADER) {
        ...
        if (fBench) {
            ...
            LogInfo("UDP: Sent block data chunks in %lf ms\n", to_millis_double(finished - start));
        }
    }
}
        ...
        if (inUDPProcess) {
            ...
            if (it != mapPartialBlocks.end() && it->second->currentlyProcessing) {
                ...
                if (it->second->block_data.AreChunksAvailable()) {
                    if (fBench)
                        LogInfo("UDP: Building FEC chunks from decoded block\n");
                    skipEncode = true;
            ...
            if (block.is_header_processing) {
                ...
                try {
                    ...
                    if (process_block.first.second == TRUSTED_PEER_DUMMY)
                        LogInfo("UDP: Failed to decode received header and short txids from trusted peer(s), check your trusted peers are behaving well.\n");
                    else {
                        LogInfo("UDP: Failed to decode received header and short txids from %s, disconnecting\n", process_block.first.second.ToStringAddrPort());
                ...
                if (decode_status != READ_STATUS_OK) {
                    ...
                    if (decode_status == READ_STATUS_INVALID) {
                        if (process_block.first.second == TRUSTED_PEER_DUMMY)
                            LogInfo("UDP: Got invalid header and short txids from trusted peer(s), check your trusted peers are behaving well.\n");
                        else {
                            LogInfo("UDP: Got invalid header and short txids from %s, disconnecting\n", process_block.first.second.ToStringAddrPort());
                            const auto it = mapUDPNodes.find(process_block.first.second);
                            if (it != mapUDPNodes.end())
                                DisconnectNode(it);
                        }
                    } else
                        LogInfo("UDP: Failed to read header and short txids\n");
                ...
                if (status != READ_STATUS_OK) {
                    ...
                    if (status == READ_STATUS_INVALID) {
                        if (process_block.first.second == TRUSTED_PEER_DUMMY)
                            LogInfo("UDP: Unable to decode block from trusted peer(s), check your trusted peers are behaving well.\n");
                        else {
                            const auto it = mapUDPNodes.find(process_block.first.second);
                            if (it != mapUDPNodes.end())
                                DisconnectNode(it);
                    ...
                    if (!chainman->ProcessNewBlock(pdecoded_block, force_requested, /*min_pow_checked=*/true, &fNewBlock)) {
                        ...
                        LogInfo("UDP: Failed to decode block %s\n", decoded_block.GetHash().ToString());
                        std::lock_guard<std::recursive_mutex> udpNodesLock(cs_mapUDPNodes);
                        if (have_prev) {
                            setBlocksReceived.insert(process_block.first);
                        } else {
                ...
                while (!fDone) {
                    ...
                    if (res != READ_STATUS_OK) {
                        ...
                        if (res == READ_STATUS_INVALID) {
                            if (process_block.first.second == TRUSTED_PEER_DUMMY)
                                LogInfo("UDP: Unable to process mempool for block %s from trusted peer(s), check your trusted peers are behaving well.\n", blockHash.ToString());
                            else {
                                LogInfo("UDP: Unable to process mempool for block %s from %s, disconnecting\n", blockHash.ToString(), process_block.first.second.ToStringAddrPort());
                                const auto it = mapUDPNodes.find(process_block.first.second);
                                if (it != mapUDPNodes.end())
                                    DisconnectNode(it);
                            }
                        } else
                            LogInfo("UDP: Unable to process mempool for block %s, dropping block\n", blockHash.ToString());
                        setBlocksReceived.insert(process_block.first);
                        RemovePartialBlock(process_block.first);
                        break;
                    } else {
                ...
                if (lock && !more_work)
                    lock.unlock();
                LogInfo("UDP: Initialized block %s with %ld/%ld mempool-provided chunks (or more)\n", blockHash.ToString(), mempool_provided_chunks, total_chunk_count);
            }
        } while (more_work);
    }
}
...
PartialBlockData::PartialBlockData(const CService& node, CTxMemPool* mempool, const UDPMessage& msg, const std::chrono::steady_clock::time_point& packet_recv) :
...
bool HandleBlockTxMessage(UDPMessage& msg, size_t length, const CService& node, UDPConnectionState& state, const std::chrono::steady_clock::time_point& packet_process_start, const node::NodeContext* const node_context) {
    ...
    if (msg.msg.block.obj_length > MAX_BLOCK_SERIALIZED_SIZE * MAX_CHUNK_CODED_BLOCK_SIZE_FACTOR) {
        LogInfo("UDP: Got massive obj_length of %u\n", msg.msg.block.obj_length);
        return false;
    }
    ...
    if ((msg.header.msg_type & UDP_MSG_TYPE_TYPE_MASK) == MSG_TYPE_BLOCK_CONTENTS && !block.initialized) {
        if (!block.Init(msg)) {
            LogInfo("UDP: Got block contents that couldn't match header for block id %lu\n", msg.msg.block.hash_prefix);
            return true;
        }
    ...
    if (msg.msg.block.obj_length  != block.obj_length) {
        ...
        LogInfo("UDP: Got wrong obj_length/chunsk_sent for block id %lu from peer %s! Check your trusted peers are behaving well\n", msg.msg.block.hash_prefix, node.ToStringAddrPort());
        return true;
    }
    ...
    if (!block.decoder.ProvideChunk(msg.msg.block.data, msg.msg.block.chunk_id)) {
        // Bad chunk id, maybe FEC is upset? Don't disconnect in case it can be random
        LogInfo("UDP: FEC chunk decode failed for chunk %d from block %lu from %s\n", msg.msg.block.chunk_id, msg.msg.block.hash_prefix, node.ToStringAddrPort());
        return true;
    }
