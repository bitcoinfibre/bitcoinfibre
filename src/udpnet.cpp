...
static void OpenLocalDeviceConnection();
...
static void AddConnectionFromString(const std::string& node, bool fTrust) {
    ...
    if (host_port_end == std::string::npos || local_pass_end == std::string::npos || (remote_pass_end != std::string::npos && group_end != std::string::npos)) {
        LogInfo("UDP: Failed to parse parameter to -add[trusted]udpnode: %s\n", node);
        return;
    }
    ...
    if (!addr.has_value()) {
        LogInfo("UDP: Failed to lookup hostname for -add[trusted]udpnode: %s\n", host_port);
        return;
    }
...
bool InitializeUDPConnections(node::NodeContext* const node_context) {
    ...
    if (std::get<0>(local_write_device)) {
        ...
        if (fd < 0) {
            LogInfo("Failed to open -fecwritedevice, not running any FIBRE connections\n");
            event_base_free(event_base_read);
            CloseSocketsAndReadEvents();
            return false;
        }
...
static void read_socket_func(evutil_socket_t fd, short event, void* arg) {
    ...
    if (res < 0) {
        int err = errno;
        LogInfo("UDP: Error reading from socket: %d (%s)!\n", err, NetworkErrorString(err));
        return;
    }
                ...
                txn_to_send.reserve(send_txn);
                {
                    ...
                    while (txn_to_send.size() < send_txn) {
                        ...
                        for (const auto& entry_ref : chunk_entries) {
                            ...
                            std::vector<CTransactionRef> to_add{entry.GetSharedTx()};
                            while (!to_add.empty() && txn_to_send.size() < send_txn) {
                                ...
                                for (const CTxIn& txin : txref->vin) {
                                    ...
                                    if (it != g_node_context->mempool->mapTx.end() && !txids_to_send.count(txin.prevout.hash)) {
...
static std::tuple<int64_t, bool, std::string> get_local_device() {
    ...
    if (bw_end == std::string::npos || backfill_end == std::string::npos) {
        LogInfo("Failed to parse -fecwritedevice=bw,backfill,file option, not writing\n");
        return std::make_tuple((int64_t)0, false, std::string());
    }

    std::string backfill_str(localUDPWriteDevice.substr(bw_end + 1, backfill_end - bw_end - 1));
    if (backfill_str != "true" && backfill_str != "false") {
        LogInfo("-fecwritedevice=bw,backfill,file backfill option must be true or false, not writing\n");
        return std::make_tuple((int64_t)0, false, std::string());
    }

    int64_t bw = LocaleIndependentAtoi<int64_t>(localUDPWriteDevice.substr(0, bw_end));
...
static void OpenUDPConnectionTo(const CService& addr, const UDPConnectionInfo& info) {
    ...
    bool fIsLocal = (addr == LOCAL_WRITE_DEVICE_SERVICE);
...
static void OpenLocalDeviceConnection() {
    OpenPersistentUDPConnectionTo(LOCAL_WRITE_DEVICE_SERVICE, LOCAL_DEVICE_CHECKSUM_MAGIC, LOCAL_DEVICE_CHECKSUM_MAGIC, false,
            UDP_CONNECTION_TYPE_OUTBOUND_ONLY, LOCAL_SEND_GROUP);
}
